...
/**
 * Build the prompt-delivery URL for `deliverPrompt`. On the chat surface it
 * preserves an active conversation deep link (`?id=`) so inserting a prompt
 * from the library / schema explorer prefills the CURRENT conversation — the
 * chat surface reads a missing `?id=` as "new chat" and would otherwise wipe
