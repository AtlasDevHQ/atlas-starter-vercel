...
const previewRoute = createRoute({
  ...
  description:
    ...
    "Takes a predicate SURFACE, never a predicate key. For the two cardinality kinds that surface is resolved through the workspace's approved predicate-alias closure — the same `loadClaimVocabulary` + `slotKey` composition `POST /cardinality` runs before it writes — so THE COUNT IS FOR THE RESOLVED SLOT, not for the spelling you sent. Previewing `is priced at` where `is priced at → priced at` is in force counts pairs in `priced at`, which is exactly the slot the write lands in. "
    + "Before #5466 this route normalized without the closure while `/cardinality` closed over it, so the two agreed for an unaliased predicate and named DIFFERENT slots for an aliased one — a count for one slot beside a write to another. Send the picked surface to both routes; do not pre-resolve it. "
    + "A workspace whose closure is half-rebuilt (an approved edge with no closure row) is REFUSED rather than counted, on the same grounds `/cardinality` refuses it: keying a norm to itself is byte-identical to 'approved nothing', so a count computed against it would be confidently wrong and silent.",
  request: {
    body: {
