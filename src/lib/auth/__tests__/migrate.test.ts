...
describe("migrateAuthTables", () => {
  it("runs internal DB migration when DATABASE_URL is set", async () => {
    ...
    // migrateInternalDB: 3 audit_log + 4 conversations/messages + 2 starred column + 3 slack + 5 action_log + 2 source tracking + 7 scheduled_tasks + 1 delivery_status ALTER + 1 connections + 3 token_usage + 5 invitations = 36 queries
    // + 1 loadSavedConnections SELECT = 37 total
    expect(queries.length).toBe(37);
  ...
  it("only runs once (idempotent guard)", async () => {
    ...
    // Internal DB migration runs once (36 queries) + 1 loadSavedConnections SELECT + 1 ALTER TABLE for password_change_required (managed mode)
    expect(queries.length).toBe(38);
