...
import {
  ...
  type ExportedBrainVocabularyProposal,
  type ExportedBrainPredicateCardinality,
...
export async function exportWorkspaceBundle(
  ...
  const [
    ...
    // The alias queue + its permanent rejection memory (#5023, exported by
    // #5113). Every status EXCEPT `applying`: a `rejected` row is the human
    // decision that stops a producer re-writing what a human removed (#4507),
    // an `approved` row is the pair's history beside the edge it produced, and
    // the pending queue re-derives but costs nothing to carry. `applying` is
    // structurally uncommittable today (claim/apply/stamp share one
    // transaction) — the predicate is belt-and-braces so a future split of
    // that transaction cannot ship a foreign claim token. `claimed_at` and the
    // generated pair columns are deliberately not projected: the first is a
    // region-local token, the second are re-generated by the destination's own
    // GENERATED ALWAYS columns.
    pool.query(
      `SELECT slot_position, from_norm, to_norm, directed, source_class, confidence,
              status, proposed_by, proposed_at, reviewed_by, reviewed_at
       FROM brain_vocabulary_proposal
       WHERE ${scopeClause("workspace_id", orgScope)} AND status <> 'applying'
       ORDER BY slot_position, from_norm, to_norm ASC`,
      params,
    ),
    // Canonical-predicate cardinality decisions (#5027, exported by #5113).
    // The WHOLE row, every status: the curated `single` entries are what
    // license supersession, a stored `multi` is a human declining the
    // question, and a `rejected` row is the producers' memory. The key is the
    // SOURCE region's canonical predicate — the importer screens it against
    // the destination's post-merge closure rather than this query guessing.
    pool.query(
      `SELECT predicate_key, cardinality, status, source_class,
              proposed_by, proposed_at, reviewed_by, reviewed_at
       FROM brain_predicate_cardinality
       WHERE ${scopeClause("workspace_id", orgScope)}
  ...
  const brainVocabularyProposals: ExportedBrainVocabularyProposal[] =
    vocabularyProposalResult.rows.map((r) => ({
      // Cast rather than narrowed, on `brainVocabularyEdges`' reasoning above:
      // this reads our own table, where the slot-position CHECK is the
      // guarantee; the import reads a foreign bundle and narrows there.
      slotPosition: r.slot_position as ExportedVocabularySlotPosition,
      fromNorm: r.from_norm as string,
      toNorm: r.to_norm as string,
      directed: r.directed === true,
      sourceClass: r.source_class as string,
      confidence: r.confidence as number,
      status: r.status as string,
      // Not defaulted to `""`, on `excludedBy`'s reasoning: the column is NOT
      // NULL, so a null here is a shape change, and `""` would land a decision
      // nobody can be shown to have authored. Let the import's validation
      // refuse it.
      proposedBy: r.proposed_by as string,
      proposedAt: toISO(r.proposed_at),
      // `null` is a real value on BOTH: a machine decision (reviewedBy) and an
      // undecided row (reviewedAt). Carried verbatim, never re-stamped — the
      // destination re-stamping a review date would assert a reading it never
      // took (`brain_actor_identity`'s rule, one section over).
      reviewedBy: (r.reviewed_by as string | null) ?? null,
      reviewedAt: toISOOrNull(r.reviewed_at),
    }));

  const brainPredicateCardinalities: ExportedBrainPredicateCardinality[] =
    predicateCardinalityResult.rows.map((r) => ({
  ...
  const bundle: ExportBundle = {
    manifest: {
      ...
      counts: {
        ...
        brainVocabularyProposals: brainVocabularyProposals.length,
        brainPredicateCardinalities: brainPredicateCardinalities.length,
      },
    },
    conversations,
    semanticEntities,
