...
describe("internal DB module", () => {
  ...
  describe("migrateInternalDB()", () => {
    it("executes CREATE TABLE and CREATE INDEX statements for all tables", async () => {
      ...
      expect(calls.queries.length).toBe(38);
      ...
      expect(calls.queries[26].sql).toContain("delivery_status");
      expect(calls.queries[27].sql).toContain("CREATE TABLE IF NOT EXISTS connections");
      expect(calls.queries[28].sql).toContain("CREATE TABLE IF NOT EXISTS token_usage");
      expect(calls.queries[29].sql).toContain("idx_token_usage_user_id");
      expect(calls.queries[30].sql).toContain("idx_token_usage_created_at");
      expect(calls.queries[31].sql).toContain("CREATE TABLE IF NOT EXISTS invitations");
      expect(calls.queries[32].sql).toContain("idx_invitations_email");
      expect(calls.queries[33].sql).toContain("idx_invitations_token");
      expect(calls.queries[34].sql).toContain("idx_invitations_status");
      expect(calls.queries[35].sql).toContain("idx_invitations_pending_email");
    });
