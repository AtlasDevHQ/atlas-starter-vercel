...
export const PURGE_TABLE_DECISIONS = {
  ...
  demo_anonymous_sessions: { decision: "purged", reason: "The anonymous demo principal's ledger (#5604) — one row per minted MCP demo identity, pinned to the demo workspace by `workspace_id`. In practice that workspace is the operator's own (`ATLAS_DEMO_WORKSPACE_SLUG`), so a tenant purge never matches a row; but the column IS a workspace scope, and the tripwire is right that a scoped table gets a purge-reaching decision rather than the `platform` label `demo_leads` carries (which has no workspace column). Holds no personal data — a hashed IP, a client label, counts and timestamps; the email a visitor may hand over lives in `demo_leads`." },
