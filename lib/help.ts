...
export const SUBCOMMAND_HELP: Record<string, SubcommandHelp> = {
  ...
  datasource: {
    ...
    usage: "datasource <command> [id] [options]",
    subcommands: [
      ...
      { name: "create <id>", description: "Provision a new datasource (connection URL captured on stdin / env, never a flag)" },
      {
        name: "profile <id>",
    ...
    flags: [
      ...
      { flag: "--description <s>", description: "(create) Human-readable description" },
    ...
    examples: [
      ...
      "atlas datasource create prod-us            # prompts for the URL on stdin",
      "ATLAS_DATASOURCE_SECRET=postgres://… atlas datasource create prod-us   # headless",
      "atlas datasource profile prod-us",
    ...
    notes: [
      ...
      "`create` reads the secret-bearing connection URL from the ATLAS_DATASOURCE_SECRET",
      "env var (headless agents) or a hidden stdin prompt (interactive) — NEVER from a",
      "flag, so it stays out of argv, shell history, and logs. CI with no terminal and no",
      "env var defers datasource creation to the dashboard or the Atlas MCP (ADR-0025 §4).",
      "The new datasource lands as a draft; publish it in the Atlas console.",
      "`profile` is long-running: it streams per-table progress and is cancellable (Ctrl-C).",
      "Generated entities land as DRAFTS — publish them from the admin console.",
