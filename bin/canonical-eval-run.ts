...
// Value import, not just a re-export: this file CALLS `writeFdSync` in four
// places, and `export … from` binds nothing locally.
...
// `writeFdSync` moved to its own leaf module in #5041 so a second eval driver
// could take it without taking this file's module graph (`./atlas`,
// `./canonical-eval`) with it. Re-exported here because the fixtures and the
// fd-1 guard suite import it from this path, and because `humanWriter` below
// is its principal caller.
export { writeFdSync };
